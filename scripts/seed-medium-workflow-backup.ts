...
/**
 * Medium Workflow Seeding Script
 * 
 * Creates a medium complexity workflow structure:
 * [Region Subgroup] (Employee → Manager) → [Parallel Split] (Legal | Finance | IT) → 
 * [Parallel Join] → [Enterprise Group] (CEO) → [Option Node] (Investor | Board | Govt)
 */
...
async function createMediumWorkflow(companyId: any, roles: CreatedRoles, groups: CreatedGroups): Promise<any> {
  console.log('Creating medium complexity workflow...');
  ...
  const nodes = [
    ...
    // Parallel branches - Legal, Finance, IT
    {
      id: 'legal-1',
      type: 'approval',
      label: 'Legal',
    ...
    // Option Node for final approvals
    {
      id: 'option-1',
      type: 'options',
  ...
  workflow = new WorkflowConfiguration({
    name: 'Medium Approval Workflow',
    description: 'Medium complexity workflow with parallel processing and conditional routing',
    companyId: companyId,
    nodes,
    edges,
    isActive: true,
  ...
  await workflow.save();
  console.log('  ✓ Created medium workflow');
...
async function seedMediumWorkflow() {
  try {
    console.log('🌱 Starting medium workflow seeding...\n');
    ...
    console.log('  [Region] Employee → Manager → [Parallel] Legal|Finance|IT → [Join] → [Enterprise] CEO → [Option] Investor|Board|Govt → End');
    console.log('\nRoles created:');
    console.log('  • Employee (Requester) - Regional employee');
    console.log('  • Manager (Forwarder) - Regional manager');
    console.log('  • Legal (Approver) - Legal compliance specialist');
    console.log('  • Finance (Approver) - Financial reviewer');
    console.log('  • IT (Approver) - Technical specialist');
    console.log('  • CEO (Approver) - Enterprise-level executive');
    console.log('  • Investor (Final Approver) - Investment decisions');
    console.log('  • Board (Final Approver) - Strategic decisions');
    console.log('  • Government (Final Approver) - Regulatory compliance');
    console.log('\nGroups created:');
    console.log('  • Region - Contains Employee and Manager roles');
    console.log('  • Enterprise - Contains CEO role');
    console.log('\nWorkflow features:');
    console.log('  • Regional subgrouping for employees and managers');
    console.log('  • Parallel processing through Legal, Finance, and IT');
    console.log('  • Conditional routing to appropriate final approver');
    console.log('  • Enterprise-level CEO approval before final routing');
    
    console.log(`\nSample users created: ${users.length} users`);
    console.log('Default password for all users: password123');
    console.log('\nSignup forms configured for all roles with appropriate group selections.');
    ...
    for (const user of users) {
      ...
      const roleName = populatedUser?.role?.name || 'Unknown';
    ...
    console.log('🚀 The medium workflow is now ready for testing with parallel processing and conditional routing.');
